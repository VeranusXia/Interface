...
function BFU_GetServerName(unit)
...
if (GetLocale() == "zhCN") then
	...
	BigFootBank_SayTitle="黑市数据已刷新"
	BigFootBank_LogoutSayTitle ="黑市竞标物品：%s  剩余时间为：%s";

	BigFootBank_MINIMAP_TOOLTIP1="黑市助手"
	BigFootBank_MINIMAP_TOOLTIP2="点击打开黑市界面"

	BigFootBank_OffLineTime="黑市数据更新时间："
	BigFootBank_OffLineNoData="沒有黑市数据。"
elseif (GetLocale() == "zhTW") then
	BigFootBank_BFchannel="大腳世界頻道";
	BigFootBank_SayTitle="黑市數據已刷新"
	BigFootBank_LogoutSayTitle="你的黑市拍賣行有物品:%s 剩餘時間為：%s"

	BigFootBank_MINIMAP_TOOLTIP1="黑市助手"
	BigFootBank_MINIMAP_TOOLTIP2="点击打开黑市界面"

	BigFootBank_OffLineTime="黑市數據更新時間："
	BigFootBank_OffLineNoData="沒有黑市數據。"
else
...
local function BigFootBank_GetData()
	...
	if BigFootBankSaveData[realmName].bankData.numItems and BigFootBankSaveData[realmName].bankData.numItems >0 then
		...
		local hour, minute = GetGameTime();
		local date = C_DateAndTime.GetCurrentCalendarTime(); --C_Calendar.GetDate()
