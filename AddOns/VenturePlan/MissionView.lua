...
local function Predictor_OnClick(self)
	...
	if rngModel then
		GameTooltip:SetText(hprefix .. "不稳定的诅咒", 1, 0.20, 0)
	else
		GameTooltip:SetText(hprefix .. (sim.won and "胜利" or "失败"), 1,1,1)
	end
	if incompleteModel then
		GameTooltip:AddLine("部分新随从技能没有收录|n ", 0.9,0.25,0.15)
	end
	...
	if rngModel then
		...
		if sim.mass == 1 then
			if sim.won then
				...
				if troopCount > 0 then
					GameTooltip:AddDoubleLine("小兵", troopHealth .. "/" .. troopHealthMax, 1,1,1, troopHealth > 0 and 0 or 1, troopHealth > 0 and 1 or 0.3, 0.15)
				end
			else
				local thp, mhp = 0, 0
				for i=5,12 do
				...
				local c = NORMAL_FONT_COLOR
				GameTooltip:AddLine("回合数: |cffffffff" .. sim.turn, c.r, c.g, c.b)
				GameTooltip:AddLine("敌方剩余血量: |cffffffff" .. thp .. " (" .. math.ceil(thp/mhp*100) .. "%)", c.r, c.g, c.b)
			end
		elseif sim.exhaustive and (sim.pWin == 1 or sim.pLose == 1) and sim.forks then
			...
			if turns then
				GameTooltip:AddLine((sim.won and "回合数: |cffffffff" or "回合数: |cffffffff") .. turns, c.r, c.g, c.b)
			end
			if sim.won then
				...
				if troopCount > 0 then
					...
					local chp = hmin == hmax and hmin or ((hmin == 0 and "|cffff40200|r" or hmin) .. " |cffffffff-|r " .. hmax)
					GameTooltip:AddDoubleLine("小兵", chp .. "/" .. troopHealthMax, 1,1,1, troopHealth2 > 0 and 0 or 1, troopHealth2 > 0 and 1 or 0.3, 0.15)
				end
			else
				...
				GameTooltip:AddLine("敌方剩余血量: |cffffffff" .. chp .. " (" .. cr .. "%)", c.r, c.g, c.b)
			end
		end
		if not incompleteModel then
...
local function MissionRewards_OnShow(self)
	...
	self.Duration:SetText(d and "耗时: |cffffffff" .. d or "")
