...
local function Predictor_OnClick(self)
	...
	if (sim.pWin < 1 and sim.pWin > 0) or not sim.exhaustive then
		GameTooltip:SetText("不稳定的诅咒", 1, 0.20, 0)
		if sim.exhaustive then
			...
			elseif sim.pLose == 0 then
				GameTooltip:AddLine("不出意外你要赢了", 1,1,1,1)
			end
		end
		GameTooltip:AddLine('"运气好的话,这根本输不了"', 1, 0.835, 0.09, 1)
	else
		...
		GameTooltip:AddLine('"加油,打工人!"', 1, 0.835, 0.09, 1)
	end
