...
local LDB = LibStub("LibDataBroker-1.1"):NewDataObject("MethodDungeonTools", {
	...
	OnTooltipShow = function(tooltip)
		...
		tooltip:AddLine("单击以切换配置窗口")
		tooltip:AddLine("右键单击以锁定迷你地图按钮")
	end,
})
...
do
    for i=1,26 do
        defaultSavedVars.global.presets[i] = {
            [1] = {text="默认",value={},colorPaletteInfo={autoColoring=true,colorPaletteIdx=1}},
            [2] = {text="<新预设>",value=0},
        }
...
function MethodDungeonTools:ToggleDataCollection()
    ...
    print(string.format("%sMDT|r: 资料收集%s，请重新载入界面!",methodColor,db.dataCollectionActive and "|cFF00FF00已启用|r" or "|cFF00FF00已停用|r"))
end

function MethodDungeonTools:ToggleHealthTrack()
    MethodDungeonTools.DataCollection:InitHealthTrack()
    print(string.format("%sMDT|r: 治疗追踪 %s.",methodColor,"|cFF00FF00已启用|r"))
end
...
function MethodDungeonTools:MakeTopBottomTextures(frame)
    ...
    frame.bottomPanelString = frame.bottomPanel:CreateFontString("MethodDungeonTools 版本")
	...
	frame.bottomPanelString:SetText("v"..GetAddOnMetadata(AddonName, "Version"))--.." - 请在 discord.gg/nnogga 或 github.com/nnogga/MethodDungeonTools 上报缺少/位置错误的 NPC"
...
function MethodDungeonTools:MakeSidePanel(frame)
	...
	frame.sidePanelNewButton:SetText("新建")
    ...
    frame.sidePanelNewButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine("创建一个新的预设",1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.sidePanelRenameButton:SetText("重命名")
    ...
    frame.sidePanelRenameButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine("重命名预设",1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.sidePanelImportButton:SetText("导入")
    ...
    frame.sidePanelImportButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine("从文本字符串导入预设",1,1,1)
        GameTooltip:AddLine("您可以在wago.io网站上找到其他用户的MDT导出",1,1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.sidePanelExportButton:SetText("导出")
    ...
    frame.sidePanelExportButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine("将预设导出为文本字符串",1,1,1)
        GameTooltip:AddLine("您可以在wago.io网站上共享MDT导出",1,1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.sidePanelDeleteButton:SetText("删除")
	...
	frame.sidePanelDeleteButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine("删除这个预设",1,1,1)
        GameTooltip:AddLine("按住Shift键并单击以删除此副本的所有预设",1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.sidePanelDeleteButton:SetCallback("OnClick",function(widget,callbackName,value)
        if IsShiftKeyDown() then
            ...
            local prompt = "!!警告!!\n是否要删除当前地下城的所有预设?\n您将要删除 "..numPresets.." 预设.\n这是无法撤消的\n\n"
            MethodDungeonTools:OpenConfirmationFrame(450,150,"清除所有预设","Delete",prompt, MethodDungeonTools.DeleteAllPresets)
        else
            MethodDungeonTools:HideAllDialogs()
            ...
            local currentPresetName = db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].text
            frame.DeleteConfirmationFrame.label:SetText("删除 "..currentPresetName.."?")
            frame.DeleteConfirmationFrame:Show()
        end
	end)

	frame.LinkToChatButton = AceGUI:Create("Button")
	frame.LinkToChatButton:SetText("分享")
	...
	frame.LinkToChatButton:SetCallback("OnClick",function(widget,callbackName,value)
        ...
        if presetSize>25000 then
            local prompt = "您正在尝试共享一个很大的预设 ("..presetSize.." 角色)\n建议使用导出功能并通过wago.io共享大型预设.\n您确定要分享此预设吗?\n"
    ...
    frame.LinkToChatButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine("与您的小队成员共享预设",1,1,1)
        GameTooltip:Show()
    end)
    ...
    frame.ClearPresetButton:SetText("清除")
    ...
    frame.ClearPresetButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine("将预设重置为默认状态",1,1,1)
        GameTooltip:AddLine("不删除您的工程图",1,1,1)
        GameTooltip:Show()
    end)
    ...
    frame.LiveSessionButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine("启动或加入当前 |cFF00FF00实时对话|r",1,1,1)
        GameTooltip:AddLine("单击此按钮将尝试加入正在进行的群组的实时会话，或者如果找不到任何会话，则创建一个新的实时会话。",1,1,1,1)
        GameTooltip:AddLine("预设将在参与实时会话的所有参与方成员之间持续同步",1,1,1,1)
        GameTooltip:AddLine("玩家可以通过单击此按钮或“实时会话”聊天链接来加入实时会话",1,1,1,1)
        GameTooltip:AddLine("要在实时会话处于活动状态时共享其他预置，只需导航至首选预置，然后单击预置下拉菜单旁边的新“设置为实时”按钮",1,1,1,1)
        GameTooltip:AddLine("您始终可以通过单击预设下拉菜单旁边的“返回实时”按钮来返回当前的实时会话预设",1,1,1,1)
        GameTooltip:Show()
    end)
    ...
    frame.MDIButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine("打开MDI覆盖选项",1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.AutomaticColorsButton:SetText("颜色")
    ...
    affixWeekWarning:SetCallback("OnEnter",function(...)
        ...
        GameTooltip:AddLine("选定的词缀不是本周的词缀",1,1,1)
        GameTooltip:AddLine("单击以切换到本周",1,1,1)
        GameTooltip:Show()
    end)
    ...
    frame.sidePanel.DifficultySlider:SetLabel("地下城等级")
	...
	frame.sidePanel.DifficultySlider:SetCallback("OnEnter",function()
        ...
        GameTooltip:AddLine("选择地下城等级",1,1,1)
        GameTooltip:AddLine("所选级别将影响显示的NPC血量",1,1,1)
        GameTooltip:AddLine("低于10的等级将隐藏与赛季词缀相关的敌人",1,1,1)
        GameTooltip:Show()
	end)
    ...
    difficultyWarning:SetCallback("OnEnter",function(...)
        ...
        GameTooltip:AddLine("所选的地下城级别低于10",1,1,1)
        GameTooltip:AddLine("当前隐藏与赛季词缀有关的敌人",1,1,1)
        GameTooltip:AddLine("单击以将地下城级别设置为10",1,1,1)
        GameTooltip:Show()
    end)
...
function MethodDungeonTools:DisplayMDISelector()
    ...
    if not MethodDungeonTools.MDISelector then
        ...
        MethodDungeonTools.MDISelector.BeguilingDropDown:SetLabel("赛季词缀:")
        local beguilingList = {[1]="迷醉 1 虚空",[2]="迷醉 2 潮汐",[3]="迷醉 3 魔法.",[13]="收割",[14]="觉醒 A",[15]="觉醒 B"}
        ...
        MethodDungeonTools.MDISelector.FreeholdDropDown:SetLabel("自由镇:")
        local freeholdList = {"1. 破浪","2. 黑齿","3. 水鼠帮"}
...
function MethodDungeonTools:UpdatePullTooltip(tooltip)
    ...
    elseif MouseIsOver(frame.sidePanel.newPullButton.frame) then
		...
		if frame.sidePanel.newPullButtons and tooltip.currentPull and frame.sidePanel.newPullButtons[tooltip.currentPull] then
			...
			for k,v in pairs(frame.sidePanel.newPullButtons[tooltip.currentPull].enemyPortraits) do
				if MouseIsOver(v) then
					if v:IsShown() then
                        ...
                        text = text.."等级 "..v.enemyData.level.." "..v.enemyData.creatureType..newLine
                        ...
                        text = text.."进度: "..MethodDungeonTools:FormatEnemyForces(count,totalForcesMax,false)
            ...
            local text = "进度: "..MethodDungeonTools:FormatEnemyForces(pullForces,totalForcesMax,false)
            text = text.. "\n总进度 :"..MethodDungeonTools:FormatEnemyForces(totalForces,totalForcesMax,true)
...
function MethodDungeonTools:OpenNewPresetDialog()
	...
	MethodDungeonTools.main_frame.PresetCreationEditbox:SetText("预设 "..countPresets+1)
...
function MethodDungeonTools:OpenClearPresetDialog()
    ...
    local currentPresetName = db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].text
    MethodDungeonTools.main_frame.ClearConfirmationFrame.label:SetText("清除 "..currentPresetName.."?")
...
function MethodDungeonTools:CreateNewPreset(name)
	if name == "<新预设>" then
		MethodDungeonTools.main_frame.presetCreationLabel:SetText("不能新建预设 '"..name.."'")
	...
	if duplicate == false then
		...
		MethodDungeonTools.main_frame.presetCreationLabel:SetText("'"..name.."' 已经存在.")
		MethodDungeonTools.main_frame.presetCreationCreateButton:SetDisabled(true)
		MethodDungeonTools.main_frame.presetCreationCreateButton.text:SetTextColor(0.5,0.5,0.5)
		MethodDungeonTools.main_frame.presetCreationFrame:DoLayout()
	end
...
function MethodDungeonTools:OpenChatImportPresetDialog(sender,preset,live)
    ...
    chatImport:SetTitle("导入预设")
    chatImport.importButton:SetText("导入")
    chatImport.live = nil
    if live then
        chatImport.importLabel:SetText("加入现场直播:\n"..sender.. ": "..dungeon.." - "..name)
...
function MethodDungeonTools:MakePresetImportFrame(frame)
	...
	frame.presetImportFrame:SetTitle("导入预设")
	...
	frame.presetImportBox:SetLabel("导入预设:")
	...
	importButton:SetCallback("OnClick", function()
		...
		if MethodDungeonTools:ValidateImportPreset(newPreset) then
			...
			frame.presetImportLabel:SetText("导入字符串无效")
		end
	end)
...
function MethodDungeonTools:MakePresetCreationFrame(frame)
	...
	frame.presetCreationFrame:SetTitle("新预设")
	...
	frame.PresetCreationEditbox:SetLabel("预设名称:")
	...
	frame.PresetCreationEditbox:SetCallback("OnEnterPressed", function(widget, event, text)
		...
		if MethodDungeonTools:SanitizePresetName(text) then
			...
			frame.presetCreationLabel:SetText("不能创建预设 '"..text.."'")
	...
	frame.presetCreationCreateButton:SetText("创建")
	...
	frame.PresetCreationDropDown:SetLabel("从这个预设档开始编辑:")
...
function MethodDungeonTools:MakeCustomColorFrame(frame)
    ...
    frame.CustomColorFrame:SetTitle("自定义调色板")
    ...
    frame.CustomColorFrame.ColorSlider:SetLabel("选择颜色数")
...
function MethodDungeonTools:MakeRenameFrame(frame)
	...
	frame.RenameFrame:SetTitle("重命名预设")
	...
	frame.RenameFrame.Editbox:SetLabel("插入新的预设名称:")
	...
	frame.RenameFrame.Editbox:SetCallback("OnEnterPressed", function(...)
		...
		if MethodDungeonTools:SanitizePresetName(text) then
			...
			frame.RenameFrame.PresetRenameLabel:SetText("无法将预设重命名为 '"..text.."'")
	...
	frame.RenameFrame.RenameButton:SetText("改名")
...
function MethodDungeonTools:MakeExportFrame(frame)
	...
	frame.ExportFrame:SetTitle("预设导出")
	...
	frame.ExportFrameEditbox:SetLabel("预设导出:")
...
function MethodDungeonTools:MakeDeleteConfirmationFrame(frame)
	...
	frame.DeleteConfirmationFrame:SetTitle("删除预设")
	...
	frame.DeleteConfirmationFrame.OkayButton:SetText("删除")
	...
	frame.DeleteConfirmationFrame.CancelButton:SetText("取消")
...
function MethodDungeonTools:MakeClearConfirmationFrame(frame)
	...
	frame.ClearConfirmationFrame:SetTitle("清除预设")
	...
	frame.ClearConfirmationFrame.OkayButton:SetText("清除")
	...
	frame.ClearConfirmationFrame.CancelButton:SetText("取消")
...
function MethodDungeonTools:CreateTutorialButton(parent)
    ...
    local helpPlate = {
        ...
        [1] = { ButtonPos = { x = 205,	y = 0 }, HighLightBox = { x = 0, y = 0, width = 200, height = 56 },		ToolTipDir = "RIGHT",		ToolTipText = "选择一个地下城并导航到不同的子级别" },
        [2] = { ButtonPos = { x = 205,	y = -210*scale }, HighLightBox = { x = 0, y = -58, width = (sizex-6)*scale, height = (sizey*scale)-58 },	ToolTipDir = "RIGHT",	ToolTipText = "点击选择敌人\nCTRL-单击以单选敌人\nSHIFT-单击以选择敌人并创建新的条" },
        [3] = { ButtonPos = { x = 900*scale,	y = 0*scale }, HighLightBox = { x = 838*scale, y = 30, width = 251, height = 115 },	ToolTipDir = "LEFT",	ToolTipText = "管理，共享和协作预设" },
        [4] = { ButtonPos = { x = 900*scale,	y = -87*scale }, HighLightBox = { x = 838*scale, y = 30-115, width = 251, height = 102 },	ToolTipDir = "LEFT",	ToolTipText = "自定义地下城选项s" },
        [5] = { ButtonPos = { x = 900*scale,	y = -(115+102*scale) }, HighLightBox = { x = 838*scale, y = (30-(115+102)), width = 251, height = (sidePanelHeight)+43 },	ToolTipDir = "LEFT",	ToolTipText = "创建和管理您的条\n右键单击以获取更多选项" },
    }
