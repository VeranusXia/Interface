...
local function POI_SetOptions(frame,type,poi,homeSublevel)
    ...
    if type == "nyalothaSpire" then
        ...
        frame:SetScript("OnEnter",function()
            ...
            GameTooltip:AddLine("右键单击重置NPC位置",1,1,1)
            frame.HighlightTexture:Show()
            ...
            if connectedDoor then
                ...
                GameTooltip:AddLine("\n"..npcName.." 在子级别: "..sublevelName,1,1,1)
                GameTooltip:AddLine("点击进入 "..sublevelName,1,1,1)
            end
            GameTooltip:Show()
        end)
        frame:SetScript("OnLeave",function()
    ...
    if type =="mechagonBot" then
        ...
        local botOptions = {
            [1] = {text="焊接机器人",color={r=0, g=1, b=0, a=0.8}, spellId=303952, textureId=134952}, --green
            [2] = {text="滑油机器人",color={r=1, g=0, b=0, a=0.8}, spellId=303924, textureId=252178}, --red
            [3] = {text="震击机器人",color={r=0, g=.8, b=1, a=0.8}, spellId=304063, textureId=136099}, --blue
        }
