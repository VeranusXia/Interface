...
local HeirloomsAccountWideCheckBox = settings:CreateCheckBox("战网范围",
function(self)
	self:SetChecked(true);
	self:Disable();
	self:SetAlpha(0.2);
...
local ShowMinimapButtonCheckBox = settings:CreateCheckBox("显示小地图按钮",
function(self)
...
local MinimapButtonStyleCheckBox = settings:CreateCheckBox("使用旧小地图模式",
function(self)
...
local ShowCompletedGroupsCheckBox = settings:CreateCheckBox("显示完成的组",
function(self)
...
local ShowRepeatableThingsCheckBox = settings:CreateCheckBox("跟踪可重复的任务",
function(self)
...
ShowRepeatableThingsCheckBox:SetATTTooltip("如果要将可重复的每日，每周和每年任务视为可收集，请启用此选项. 它们将像常规的收藏任务一样出现在列表中.\n\n注意: 这不打算一直使用，但是如果你在一个区域做了一套日常，你已经完成了，需要提醒你有什么，你可以用它来看看它们.");
ShowRepeatableThingsCheckBox:SetPoint("TOPLEFT", ShowIncompleteThingsCheckBox, "BOTTOMLEFT", 4, 4);

local ShowRepeatableThingsFirstTimeCheckBox = settings:CreateCheckBox("仅第一次",
function(self)
...
ShowRepeatableThingsFirstTimeCheckBox:SetATTTooltip("如果您希望将每天，每周，每年和世界范围内重复执行的任务视为已完成（至少完成一次），而忽略先前已重置的任务，则启用此选项.\n\n注意: 先前完成的可重复任务仅在您通过插件激活完成任务的情况下存储，并且如果从WTF文件夹中删除了插件数据，则数据将丢失.");
...
local ReportCollectedThingsCheckBox = settings:CreateCheckBox("庆祝已收集的东西",
function(self)
...
f:SetATTTooltip("单击此按钮一次启用所有设备过滤器.");
f.OnRefresh = function(self) 
	if settings:Get("AccountMode") or settings:Get("DebugMode") then
		self:Disable();
	else
...
f:SetATTTooltip("单击此按钮一次禁用所有设备过滤器.");
f.OnRefresh = function(self) 
	if settings:Get("AccountMode") or settings:Get("DebugMode") then
		self:Disable();
	else
...
------------------------------------------
(function()
local tab = settings:CreateTab("接口");
...
ShowDescriptionsCheckBox:SetATTTooltip("启用此选项可在鼠标提示中显示说明. 这可能包括地下城说明提供的描述性文字，或由认为有必要提供其他信息的投稿人添加的自定义描述.\n\n你可能想把这个打开.");
...
local ShowRemainingCheckBox = settings:CreateCheckBox("展示剩余的东西",
function(self)
...
ShowRemainingCheckBox:SetATTTooltip("如果要查看剩余的项目数而不是总进度，请启用此选项.");
...
MainListScaleSlider.tooltipText = '使用此选项可自定义主列表的比例.\n\n默认值: 1';
...
_G[MainListScaleSlider:GetName() .. 'Text']:SetText("主列表比例")
...
MiniListScaleSlider.tooltipText = '使用此选项可自定义所有迷你和列表的比例.\n\n默认值: 1';
...
_G[MiniListScaleSlider:GetName() .. 'Text']:SetText("迷你列表比例")
...
LocationsSlider.tooltipText = '使用此选项可自定义要在鼠标提示中显示的来源位置数.\n\n注意: 这也将根据显示元素的总数等于显示元素的总数来显示其他来源的“X”个数，然后只显示最后一个来源.\n\n默认值: 5';
...
_G[LocationsSlider:GetName() .. 'Text']:SetText("显示来源位置")
...
local AutomaticallySkipCutscenesCheckBox = settings:CreateCheckBox("自动跳过过场动画",
function(self)
...
AutomaticallySkipCutscenesCheckBox:SetATTTooltip("如果您希望ATT代表您自动跳过所有过场动画，请启用此选项.");
AutomaticallySkipCutscenesCheckBox:SetPoint("TOPLEFT", ModulesLabel, "BOTTOMLEFT", 4, 0);

local OpenBountyListAutomatically = settings:CreateCheckBox("自动打开赏金列表",
function(self)
...
OpenBountyListAutomatically:SetATTTooltip("如果要查看具有未结收藏金额的物品，请启用此选项. 如果您设法阻止此列表中发布的其中一项，您可以获得一笔不错的金额.\n\n快捷命令: /attbounty");
...
OpenMiniListAutomatically:SetATTTooltip("如果要查看当前区域中可以收集的所有内容，请启用此选项. 更改区域时，列表将自动切换. 有些人不喜欢这个功能，但是当你进行单人模式时，这个功能非常有用.\n\n您还可以将此设置绑定到快捷键.\n\nKey Bindings -> Addons -> ALL THE THINGS -> Toggle Mini List\n\n快捷命令: /att mini");
...
OpenProfessionListAutomatically:SetATTTooltip("如果您希望在打开职业时打开并刷新职业列表，请启用此选项. 由于暴雪强加的API限制，插件可以与您的专业数据进行交互的唯一时间是打开时. 当您更改为其他职业时，列表将自动切换.\n\n我们不建议禁用此选项，因为它可能会阻止配方正确跟踪.\n\n您还可以将此设置绑定到快捷键. (只有在专业开放时才有效)\n\nKey Bindings -> Addons -> ALL THE THINGS -> Toggle Profession Mini List\n\n快捷命令: /att prof");
...
OpenRaidAssistantAutomatically:SetATTTooltip("如果要查看名为“Raid助手”的组/ 队伍 / 团队设置管理器，请启用此选项. 每当组设置更改时，列表将自动更新.\n\n您还可以将此设置绑定到快捷键.\n\nKey Bindings -> Addons -> ALL THE THINGS -> Toggle Raid Assistant\n\n快捷命令: /attra");
...
OpenWorldQuestsListAutomatically:SetATTTooltip("如果您希望“世界任务”列表自动显示，请启用此选项. 切换区域时，列表将自动更新.\n\n您还可以将此设置绑定到快捷键.\n\nKey Bindings -> Addons -> ALL THE THINGS -> Toggle World Quests List\n\n快捷命令: /attwq");
OpenWorldQuestsListAutomatically:SetPoint("TOPLEFT", OpenRaidAssistantAutomatically, "BOTTOMLEFT", 0, 4);

local ShowCurrenciesInWorldQuestsList = settings:CreateCheckBox("将货币视为任务",
function(self)
...
ShowCurrenciesInWorldQuestsList:SetATTTooltip("如果您想要处理由世界任务授予的货币，就好像它们用于获取的所有物品在列表中计为+1一样，请启用此选项.");
ShowCurrenciesInWorldQuestsList:SetPoint("TOPLEFT", OpenWorldQuestsListAutomatically, "BOTTOMLEFT", 4, 4);

local ShowAuctionHouseModuleTab = settings:CreateCheckBox("显示拍卖行模块选项卡",
function(self)
...
ShowAuctionHouseModuleTab:SetATTTooltip("如果要查看随ATT提供的拍卖行模块，请启用此选项.\n\n一些插件很顽皮，并且广泛地修改了这个框架. ATT并不总能与这些玩具搭配得很好.");
...
CelebrationsLabel:SetText("庆祝活动和音效");
CelebrationsLabel:Show();
table.insert(settings.MostRecentTab.objects, CelebrationsLabel);

local UseMasterAudioChannel = settings:CreateCheckBox("使用主音频通道",
function(self)
...
UseMasterAudioChannel:SetATTTooltip("如果您希望在“MASTER”音频通道上播放庆典和其他ATT音效，请启用此选项.\n\n默认值: 开启\n\n很多人都会关闭声音效果，所以这个选项允许ATT声音在需要时绕过它.");
UseMasterAudioChannel:SetPoint("TOPLEFT", CelebrationsLabel, "BOTTOMLEFT", 4, 0);

local CelebrateCollectedThingsCheckBox = settings:CreateCheckBox("收集的东西引发庆祝",
function(self)
...
CelebrateCollectedThingsCheckBox:SetATTTooltip("如果您希望在获得新物品时听到庆祝的“大肆宣传”声音效果，请启用此选项.\n\n此功能可以极大地帮助您保持动力.");
CelebrateCollectedThingsCheckBox:SetPoint("TOPLEFT", UseMasterAudioChannel, "BOTTOMLEFT", 0, 4);

local WarnRemovedThingsCheckBox = settings:CreateCheckBox("删除东西会触发警告",
function(self)
...
WarnRemovedThingsCheckBox:SetATTTooltip("如果您想要在意外退回或交易一件物品时听到警告声音效果，请启用此选项，该物品会使您的外观从您的收藏中丢失.\n\n如果您提供带有购买计时器的商品，这将非常有用. 插件会告诉你，你犯了一个错误.");
...
AboutText:SetText(L["TITLE"] .. " |CFFFFFFFF是一个集合跟踪插件，向您显示在游戏中获取所有内容的位置和方式! 我们在Discord上有一个庞大的用户社区（在底部链接），您可以在那里提出问题，提交建议以及报告错误/缺失物品. 如果您发现某些收藏品或未记录的任务，您可以在Discord上告诉我们，或者为了更加技术化的建议，我们有一个Git，您可以直接使用.\n\n虽然我们确实努力完成，但是每个补丁中都会添加很多内容，所以如果我们遗漏了一些东西，请理解我们是一个小团队试图跟上变化并自己收集东西. :D\n\n我在流媒体时随意问我问题，我会尽力回答它，即使它与ATT没有直接关系. (一般WoW Addon编程也是如此)\n\n- |r|Cffff8000Crieve (DFortun81)|CFFFFFFFF\n\nPS: 作为一个社区，我们目前专注于Legion Raid Transmog，所以如果你对此感兴趣，我们会在亚利桑那时间周五和周六下午3点组成小组. 在这段时间内搜索Premade Group finder for“CRIEVE”，您可能会找到我们的小组!\n\n\n\n我一直在问这个问题:\n是的，会有经典魔兽世界的ATT版本. 它只是一个战利品和任务追踪器，因为很明显在Classic中没有收集幻化. (也不应该)\n\n是的，我打算玩Classic WoW，但在全职工作和开发两个版本的插件之间，不会有很多时间进行搜索.\n\n不，ATT不是在你的包包图标上放置图标的插件. 那是CanIMogIt和Caerdon Wardrobe!\n\n比较即将推出的收藏品的网站™.|r");
AboutText:Show();
