...
function BrowsePanel:OnInitialize()
        ...
        ActivityList:InitHeader{
            {
                ...
                iconHandler = function(activity)
                    ...
                    elseif activity:IsSelf() then
                        return [[Interface\AddOns\MeetingStonePlus\Media\Icons]], 0.25, 0.375, 0, 1
                    elseif activity:IsInActivity() then
                        return [[Interface\AddOns\MeetingStonePlus\Media\Icons]], 0.375, 0.5, 0, 1
                    elseif activity:IsApplication() then
                        return [[Interface\AddOns\MeetingStonePlus\Media\Icons]], 0.5, 0.625, 0, 1
                    elseif activity:IsAnyFriend() then
                        return [[Interface\AddOns\MeetingStonePlus\Media\Icons]], 0, 0.125, 0, 1
                    end
                end,
    ...
    local canUse,reason = C_LFGInfo.CanPlayerUsePremadeGroup();
    ...
    local SearchingBlocker = CreateFrame('Frame', nil, self) do
        ...
        if(canUse) then
            ...
            local Label = SearchingBlocker:CreateFontString(nil, SearchingBlocker, 'GameFontNormalLarge') do
    ...
    local IconSummary = CreateFrame('Button', nil, self) do
        ...
        local icon = IconSummary:CreateTexture(nil, 'OVERLAY') do
            icon:SetSize(20, 20)
            icon:SetPoint('LEFT')
            icon:SetTexture([[Interface\AddOns\MeetingStonePlus\Media\Icons]])
        ...
        IconSummary:SetScript('OnEnter', function(button)
            ...
            GameTooltip:AddLine(format([[|TInterface\AddOns\MeetingStonePlus\Media\Icons:20:20:0:0:256:32:64:96:0:32|t %s]], L['我的活动']), 1, 1, 1)
            GameTooltip:AddLine(format([[|TInterface\AddOns\MeetingStonePlus\Media\Icons:20:20:0:0:256:32:96:128:0:32|t %s]], L['已加入活动']), 1, 1, 1)
            GameTooltip:AddLine(format([[|TInterface\AddOns\MeetingStonePlus\Media\Icons:20:20:0:0:256:32:128:160:0:32|t %s]], L['申请中活动']), 1, 1, 1)
            GameTooltip:AddLine(format([[|TInterface\AddOns\MeetingStonePlus\Media\Icons:20:20:0:0:256:32:0:32:0:32|t %s]], L['好友或公会成员参与的活动']), 1, 1, 1)
            GameTooltip:Show()
        end)
