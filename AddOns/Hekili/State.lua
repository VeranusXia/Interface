    ...
    function state:IsUsable( spell )
        ...
        if ability.channeling then
            ...
            if not c or state.buff.casting.down or not state.buff.casting.v3 or state.buff.casting.v1 ~= c then
                return false, "需要引导 (" .. c .. " / " .. ability.channeling .. ") 未激活 [ " .. state.buff.casting.remains .. " / " .. state.buff.casting.applied .. " / " .. state.buff.casting.expires .. " / " .. state.query_time .. " / " .. tostring( state.buff.casting.v3 ) .. " / " .. state.buff.casting.v1 .. " ]"
            end
        end

        if self.args.moving == 1 and state.buff.movement.down then
            return false, "进入需要移动而玩家不移动"
        end

        if self.args.moving == 0 and state.buff.movement.up then
            return false, "进入时不需要移动而玩家在移动"
        end
        ...
        if ability.usable ~= nil then
            ...
            return false, reason or "技能可使用函数无故返回false"
        end
...
function state:IsReady( action )
    ...
    if ability.spend then
        ...
        if resource == "focus" or resource == "energy" or state.script.entry then
            ...
            if ttr <= self.delayMin or ttr >= self.delayMax then return false, "在我们的时间范围内还没有准备好"
            elseif ttr >= state.delay then return false, "在选定操作之前未准备好" end
