...
if UnitClassBase( "player" ) == "HUNTER" then
    ...
    spec:RegisterAbilities( {
        ...
        primal_rage = {
            ...
            usable = function () return pet.alive and pet.ferocity, "需要一个活着的狂野宠物" end,
            handler = function ()
        ...
        summon_pet = {
            ...
            usable = function () return not pet.exists, "需要不激活宠物" end,
        ...
        tranquilizing_shot = {
            ...
            usable = function () return buff.dispellable_enrage.up or buff.dispellable_magic.up, "需要怒气或魔法效果" end,
            handler = function ()
        ...
        serenity_dust = {
            ...
            usable = function ()
                if not pet.alive then return false, "需要一个活的宠物" end
                return buff.dispellable_enrage.up or buff.dispellable_magic.up, "需要怒气或魔法debuff"
            end,
            handler = function ()
                removeBuff( "dispellable_enrage" )
                removeBuff( "dispellable_magic" )
        ...
        wild_spirits = {
            ...
            disabled = function ()
                return covenant.night_fae and not IsSpellKnownOrOverridesKnown( 328231 ), "你没有完成你的night_fae盟约引导"
            end,
    ...
    spec:RegisterSetting( "barbed_shot_grace_period", 0.5, {
        name = "|T2058007:0|t倒刺射击宽限期",
        desc = "如果设置为零以上, 则插件（使用默认优先级或|cFFFFD100barbed_shot_grace_period|r表达式）将建议|T2058007:0|t倒刺射击提前1个全局冷却时间.",
        icon = 2058007,
    ...
    spec:RegisterSetting( "avoid_bw_overlap", false, {
        name = "避免 |T132127:0|t 狂野怒火重叠",
        desc = "如果选中, 如果已经有|T132127:0|t 狂野怒火的buff插件将不会推荐|T132127:0|t 狂野怒火.",
        type = "toggle",
        width = "full"
    } )
