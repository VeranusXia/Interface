...
if UnitClassBase( "player" ) == "ROGUE" then
    ...
    spec:RegisterAbilities( {
        ...
        shadow_dance = {
            ...
            usable = function () return not stealthed.all, "潜行无法使用" end,
            handler = function ()
        ...
        shadowstrike = {
            ...
            usable = function () return stealthed.all or buff.sepsis_buff.up, "需要潜行或sepsis_buff" end,
            handler = function ()
                removeBuff( "cold_blood" )
        ...
        stealth = {
            ...
            usable = function ()
                if time > 0 then return false, "战斗中无法使用" end
                if buff.stealth.up then return false, "前行无法使用" end
                if buff.vanish.up then return false, "消失时无法使用" end
                return true
            end,
        ...
        tricks_of_the_trade = {
            ...
            usable = function () return group, "需要盟友" end,
            handler = function ()
    ...
    spec:RegisterSetting( "priority_rotation", false, {
        name = "使用优先循环 (汇集伤害)",
        desc = "如果勾选, 默认优先推荐使用 |T1375677:0|t 袖剑风暴 建立连击点并将终结技花费在单目标上.",
        type = "toggle",
        width = "full"
    })
