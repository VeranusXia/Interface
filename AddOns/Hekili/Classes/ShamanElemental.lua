...
if UnitClassBase( "player" ) == "SHAMAN" then
    ...
    --[[ spec:RegisterSetting( "funnel_damage", false, {
        name = "注入AOE -> 目标",
        desc = function ()
            local s = "如果勾选, 插件的默认优先级将鼓励你传播|T135813:0|t烈焰震击, 但会将伤害集中在你当前的目标上, 使用|T136026:0|t大地震击而不是|T451165:0|t地震术."

            if not Hekili.DB.profile.specs[ state.spec.id ].cycle then
                s = s .. "\n\n|cFFFF0000要求在目标选项卡上显示'推荐目标交换'.|r"
            end
    ...
    spec:RegisterSetting( "stack_buffer", 1.1, {
        name = "|T135855:0|t冰怒和|T839977:0|t风暴守护者填充",
        desc = "默认优先级试图避免浪费|T839977:0|t风暴守护者和|T135855:0|t冰怒层数，每个层数的宽限期为1.1 GCD.\n\n" ..
                "增加这个数量将减少由于其他进程优先而浪费冰怒/风暴守护者层数的可能性，并让你有更多的时间做出反应.",
        type = "range",
        min = 1,
        max = 2,
        step = 0.01,
