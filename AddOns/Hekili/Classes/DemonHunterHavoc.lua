...
if UnitClassBase( "player" ) == "DEMONHUNTER" then
    ...
    do
        ...
        spec:RegisterEvent( "PLAYER_REGEN_DISABLED", function ()
            if state.talent.demon_blades.enabled and not state.settings.demon_blades_acknowledged and not wasWarned then
                Hekili:Notify( "|cFFFF0000警告!|r  恶魔之刃不可预测.\n查看/hekili > Havoc的更多信息." )
                wasWarned = true
            end
        end )
    end
    ...
    spec:RegisterAbilities( {
        ...
        fel_rush = {
            ...
            usable = function ()
                if settings.recommend_movement ~= true then return false, "fel_rush移动被禁用" end
        ...
        vengeful_retreat = {
            ...
            usable = function ()
                if settings.recommend_movement ~= true then return false, "vengeful_retreat移动被禁用" end
                return true
            end,
    ...
    spec:RegisterSetting( "recommend_movement", true, {
        name = "推荐移动",
        desc = "如果勾选, 插件会推荐 |T1247261:0|t 邪能冲撞 / |T1348401:0|t 复仇回避, 当它是一个潜在的DPS增益时\n\n" ..
            "在使用势如破竹天赋时这些技能对于DPS来说至关重要.\n\n" ..
            "如果不使用势如破竹, 你可能想让这个功能失效, 以避免在战斗中不必要的移动.",
        type = "toggle",
        width = "full"
    } )
    ...
    spec:RegisterSetting( "demon_blades_text", nil, {
        name = "|cFFFF0000警告!|r  如果使用|T237507:0|t 恶魔之刃天赋, 插件将无法预测你的自动攻击的恶魔之怒收益.  这将导致 " ..
            "建议在你的显示中向前跳动.",
        type = "description",
        width = "full"
    } )
    ...
    spec:RegisterSetting( "demon_blades_acknowledged", false, {
        name = "我明白恶魔之刃是不可预知的; 不要警告我.",
        desc = "如果勾选, 插件在进入战斗时不会提供关于恶魔之刃的警告.",
        type = "toggle",
        width = "full"
    } )
