...
if UnitClassBase( "player" ) == "MONK" then
    ...
    spec:RegisterAbilities( {
        ...
        breath_of_fire = {
            ...
            usable = function ()
                if active_dot.keg_smash / true_active_enemies < settings.bof_percent / 100 then
                    return false, "keg_smash应用于少于" .. settings.bof_percent .. "目标"
                end
                return true
            end,
        ...
        expel_harm = {
            ...
            usable = function ()
                if ( settings.eh_percent > 0 and health.pct > settings.eh_percent ) then return false, "血量高于 " .. settings.eh_percent .. "%" end
                return true
            end,
            handler = function ()
        ...
        --[[ ironskin_brew = {
            ...
            usable = function ()
                if not tanking and incoming_damage_3s == 0 then return false, "玩家不是坦克或者3秒内没有受到伤害" end
                return true
            end,
        ...
        purifying_brew = {
            ...
            usable = function ()
                if stagger.amount == 0 then return false, "没有醉拳伤害" end
                if health.current == 0 then return false, "你死了" end
                return true
            end,
    ...
    spec:RegisterSetting( "purify_for_celestial", true, {
        name = "最大化 |T1360979:0|t 天神酒盾",
        desc = "如果选择, 插件将专注于尽可能多的使用 |T133701:0|t 活血酒, 为你的天神酒盾堆叠净化真气.\n\n" ..
            "这很可能是酒仙轻酿天赋的最佳效果, 但风险是当你你没有活血酒充能后你的醉拳大幅飙升.\n\n" ..
            "自定义优先级可能会忽略此设置.",
        type = "toggle",
        width = "full",
    } )
    ...
    spec:RegisterSetting( "purify_stagger_currhp", 12, {
        name = "|T133701:0|t 活血酒: 醉拳每跳当前生命%",
        desc = "如果设置为零以上, 当你当前的醉拳每跳为你|cFFFF0000当前|r有效血量的这个百分比(或更多)时, 插件将推荐|T133701:0|t活血酒.  " ..
            "自定义优先级可能会忽略此设置.\n\n" ..
            "单刷时该值减半.",
        type = "range",
        min = 0,
    ...
    spec:RegisterSetting( "bof_percent", 50, {
        name = "|T615339:0|t火焰之息: 要求|T594274:0|t醉酿投%",
        desc = "如果设置为零以上，则仅当此百分比的目标受到|T594274:0|t醉酿投的影响时，才建议使用|T615339:0|t火焰之息.\n\n" ..
            "示例：如果设置为|cFFFFD10050|r, 有两个目标, 则在至少有一个目标应用醉酿投之前，将保存火焰之息.",
        type = "range",
        min = 0,
        max = 100,
        step = 0.1,
    ...
    spec:RegisterSetting( "eh_percent", 65, {
        name = "|T627486:0|t移花接木: 血量%",
        desc = "如果设置为零以上, 则在血量低于此百分比之前插件不会建议|T627486:0|t移花接木.",
        type = "range",
        min = 0,
        max = 100,
        step = 1,
    ...
    --[[ spec:RegisterSetting( "isb_overlap", 1, {
        name = "|T1360979:0|t铁骨酒: 叠加持续时间",
        desc = "如果设置为零以上，除非将要限制|T1360979:0|t铁骨酒费用，否则插件将不推荐|T1360979:0|t铁骨酒，直到buff的剩余时间少于此秒数.",
        type = "range",
        min = 0,
        max = 7,
        step = 0.1,
    ...
    spec:RegisterSetting( "brew_charges", 2, {
        name = "|T1360979:0|t铁骨酒: 存储充能",
        desc = "如果设置为零以上，插件将不推荐|T133701:0|t活血酒，如果它会让你没有|T1360979:0|t铁骨酒的充能.",
        type = "range",
        min = 0,
        max = 4,
        step = 0.1,
