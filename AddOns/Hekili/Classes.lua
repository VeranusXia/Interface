...
all:RegisterAbilities( {
    ...
    shadowmeld = {
        ...
        usable = function ()
            if not boss or group then return false, "需要打BOSS或组队(避免重置)" end
            if moving then return false, "移动时不能影遁" end
            return true
        end,
    ...
    potion = {
        ...
        usable = function ()
            ...
            if not potion or GetItemCount( potion.item ) == 0 then return false, "没有找到药水" end
    ...
    healthstone = {
        ...
        usable = function ()
            if GetItemCount( 5512 ) == 0 then return false, "背包中需要治疗石"
            elseif not IsUsableItem( 5512 ) then return false, "治疗石在CD"
            elseif health.current >= health.max then return false, "必须损坏" end
            return true
        end,
    ...
    cancel_buff = {
        ...
        usable = function () return args.buff_name ~= nil, "未检测到buff名称" end,
...
do
    ...
    all:RegisterAbility( "heart_essence", {
        ...
        usable = function () return false, "你所装备的主精华在其他地方得到支持或者不是主动技能" end
    } )
end
...
all:RegisterAbility( "shiver_venom_relic", {
    ...
    usable = function ()
        if debuff.shiver_venom.stack < 5 then return false, "shiver_venom未达到最大层数" end
        return true
    end,
    ...
    all:RegisterAbility( "ashvanes_razor_coral", {
        ...
        usable = function ()
            if active_dot.razor_coral > 0 and target.unit ~= coralGUID then
                return false, "当前目标没有应用razor_coral"
            end
            return true
        end,
...
all:RegisterAbility( "variable_intensity_gigavolt_oscillating_reactor", {
    ...
    buff = "vigor_engaged",
    usable = function ()
        if buff.vigor_engaged.stack < 6 then return false, "少于6层" end
        return true
    end,
...
do
    ...
    all:RegisterAbility( "gladiators_medallion", {
        ...
        usable = function () return debuff.loss_of_control.up, "要求失去控制效果" end,
...
local function addItemSettings( key, itemID, options )
    ...
    options.disabled = {
        type = "toggle",
        name = function () return format( "通过|cff00ccff[使用物品]|r禁用 %s", select( 2, GetItemInfo( itemID ) ) or ( "[" .. itemID .. "]" ) ) end,
        desc = function( info )
            local output = "如果禁用，加载项将不会通过|cff00ccff[使用物品]|r操作推荐此物品.  " ..
                "你仍然可以按照自己的标准将物品手动添加到操作列表中."
            return output
        end,
        order = 25,
        width = "full"
    ...
    options.minimum = {
        type = "range",
        name = "最小目标",
        desc = "只有当至少有这么多的目标可用来打的时候插件才会推荐这个物品(通过|cff00ccff[使用物品]|r).",
        order = 26,
        width = "full",
        min = 1,
        max = 10,
    ...
    options.maximum = {
        type = "range",
        name = "最大目标",
        desc = "只有当检测到的目标数量不超过这个数量时插件才会推荐这个物品(通过|cff00ccff[使用物品]|r).\n\n" ..
            "如果设置为0该设置将被忽略.",
        order = 27,
        width = "full",
        min = 0,
    ...
    for i, specID in ipairs( specs ) do
        ...
        if spec then
            if specID == currentID then
                ...
                class.potionList.default = "|cFFFFD100默认|r"
            end
