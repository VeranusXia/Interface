...
do
    ...
    function Hekili:PetBasedTargetDetectionIsReady()
        if petSlot == 0 then return false, "玩家技能栏中找不到宠物技能." end
        if not UnitExists( "pet" ) then return false, "没有激活的宠物." end
        if UnitIsDead( "pet" ) then return false, "宠物死了." end
    
        -- If we have a target and the target is out of our pet's range, don't use pet detection.
        if UnitExists( "target" ) and not IsActionInRange( petSlot, "target" ) then return false, "玩家有目标, 而玩家的目标不在宠物的范围内." end
        return true
    end
