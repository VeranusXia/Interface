...
function ns.StartConfiguration( external )
    ...
    HekiliNotification:SetScript( "OnEnter", function( self )
        ...
        if not H.Pause and H.Config then
            ...
            GameTooltip:SetText( "Hekili: 通知" )
            GameTooltip:AddLine( "左键单击并按住可移动.", 1, 1, 1 )
            GameTooltip:AddLine( "点击右键打开通知面板设置.", 1, 1, 1 )
            GameTooltip:Show()
        end
    end )
        ...
        if ns.UI.Buttons[ i ][ 1 ] and Hekili.DB.profile.displays[ i ] then
            ...
            v.Backdrop:SetScript( "OnEnter", function( self )
                ...
                if not H.Pause and H.Config then
                    ...
                    GameTooltip:AddLine( "左键单击并按住可移动.", 1, 1, 1 )
                    GameTooltip:AddLine( "点击右键打开" .. i .. "显示设置.", 1, 1, 1 )
                    if not H:IsDisplayActive( i, true, "OnEnter" ) then GameTooltip:AddLine( "此显示目前未激活.", 0.5, 0.5, 0.5 ) end
                    GameTooltip:Show()
                end
            end )
