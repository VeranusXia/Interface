...
function rematch:checkTeamHealth(team) 
	...
	if mytarget == 98489 then --海难俘虏(尾锚)
		for i=1,3 do 
			...
			if type(petID) == "string" then
				if not rematch:IsPetLeveling(petID) then
					...
					if petHP and petMaxHP then 
						if petHP < petMaxHP*0.75 then --尾锚血量设定
...
function rematch:loadSimilarTeam(key) 
	...
	if npcID then 
		...
		if team then 
			...
			local teamKey = rematch:GetTeamTitle(npcID):match("(%S-%w-)[%d-]")
			...
			if teamTitle then
				...
				if rematch:checkTeamHealth(team) then 
					...
					if needreload then
						--rematch:print(format("\124cff00ffff已加载固定队伍：%s\124r",teamTitle)) 
						needreload = false
				...
				if teamKey then
					...
					if not replaced then                   
						for k, v in pairs(saved) do 
							...
							if rematch:GetTeamTitle(k):match("(%S-%w-)[%d-]") == teamKey then 
							...
							if rematch:GetTeamTitle(k):match("(%S-%w-)[%d-]") ~= teamKey and rematch:GetTeamTitle(k):match(teamKey) then 
            		...
            		return tonumber(a.index) < tonumber(b.index)
							...
							for k, v in pairs(replaceteamlist) do
								...
								if rematch:checkTeamHealth(v.team) then 
									...
									if needreload then
										--rematch:print(format("\124cff00ffff已加载轮换队伍：%s\124r", v.id)) 
										needreload = false
					...
					if not replaced then
						if foundbkp then
							...
							if needreload then
								--rematch:print(format("\124cff00ffff已加载优势队伍：%s\124r", MyBackupTeamID))
								needreload = false
