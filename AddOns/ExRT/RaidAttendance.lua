...
function module.options:Load()
	...
	local function RadioUpdates()
		self.radioDisabled:SetChecked(not VExRT.Attendance.enabled)
		self.radioFirstTimePerRaid:SetChecked(VExRT.Attendance.enabled == 1)
		self.radioFirstPerRaidKill:SetChecked(VExRT.Attendance.enabled == 2)
		self.raidEveryPull:SetChecked(VExRT.Attendance.enabled == 3)
		self.raidEveryKill:SetChecked(VExRT.Attendance.enabled == 4)
		self.raidSpecialConditions:SetChecked(type(VExRT.Attendance.enabled) == 'string')
		if type(VExRT.Attendance.enabled) == 'string' then
			self.raidSpecialConditionsEdit:Text(VExRT.Attendance.specialEdit or "")
	...
	self.raidSpecialConditions = ELib:Radio(self,L.AttendanceSpecialConditions):Point("TOPLEFT",self.raidEveryKill,0,-25):OnClick(function(self) 
		VExRT.Attendance.enabled = VExRT.Attendance.specialEdit or ""
		RadioUpdates()
	end)
	...
	self.raidSpecialConditionsEdit = ELib:Edit(self):Size(400,20):Point("LEFT",self.raidSpecialConditions,150,0):Tooltip(L.AttendanceSelectConditionsTooltip):OnChange(function(self,isUser)
		...
		VExRT.Attendance.specialEdit = text
		VExRT.Attendance.enabled = text
	end)
	self.raidSpecialConditionsEdit:Hide()
	...
	self.DialogSetAlts:SetScript("OnHide",function(self)
		...
		wipe(VExRT.Attendance.alts)
		for i=1,#names do
			local name1,name2 = names[i]:match("([^=]+)=([^=]+)")
			if name1 and name2 then
				VExRT.Attendance.alts[#VExRT.Attendance.alts +1] = {name1,name2}
			end
		end
		UpdatePersonalData()
	end)
	function self.DialogSetAlts:OnShow()
		local data = VExRT.Attendance.alts
		local text = ""
		for i=1,#data do
	...
	self.buttonExport = ELib:Button(self,L.Export):Point("TOPLEFT",self.list,"TOPRIGHT",5,0):Size(100,20):OnClick(function()
		...
		local alts = {}
		for i=1,#VExRT.Attendance.alts do
			local nameAlt,nameMain = VExRT.Attendance.alts[i][1],VExRT.Attendance.alts[i][2]
	...
	for i=1,40 do
		...
		classButton:SetHighlightTexture( ELib:Texture(classButton,"Interface\\AddOns\\ExRT\\media\\DiesalGUIcons16x256x128"):Point("TOPLEFT",-5,2):Point("BOTTOMRIGHT",5,-2):Color(1,1,1,1):TexCoord(0.25,0.3125,0.5,0.625) )
		classButton:SetNormalTexture( ELib:Texture(classButton,"Interface\\AddOns\\ExRT\\media\\DiesalGUIcons16x256x128"):Point("TOPLEFT",-5,2):Point("BOTTOMRIGHT",5,-2):Color(1,1,1,.7):TexCoord(0.25,0.3125,0.5,0.625) )
	...
	function UpdateData()
		...
		for _,data in pairs(VExRT.Attendance.data) do
			local diff = data.d
			if diff then
	...
	function UpdatePersonalData()
		...
		for i=1,#VExRT.Attendance.alts do
			...
			if not nameToClass[ nameMain ] then
				local finded = nil
				for j=1,#VExRT.Attendance.data do
					local p = VExRT.Attendance.data[j]
					for k=1,40 do
		...
		if VExRT.Attendance.OptionsSortPD then
	...
	self.sortBy1 = ELib:Radio(self,"1",VExRT.Attendance.OptionsSortPD):Point("TOPLEFT",self.stats,"TOPRIGHT",5,-20):OnClick(function()
		VExRT.Attendance.OptionsSortPD = true
		self.sortBy1:SetChecked(true)
		self.sortBy2:SetChecked(false)
		UpdatePersonalData()
	end)
	self.sortBy2 = ELib:Radio(self,"2",not VExRT.Attendance.OptionsSortPD):Point("TOPLEFT",self.stats,"TOPRIGHT",5,-40):OnClick(function()
		VExRT.Attendance.OptionsSortPD = nil
	...
	self.rightClickDropDown.List = {
		...
		{text = DELETE,func = function(_,arg1)
			...
			for i=1,#VExRT.Attendance.data do
				if VExRT.Attendance.data[i] == data then
					tremove(VExRT.Attendance.data,i)
					break
				end
			end
			ELib.ScrollDropDown:Close()
	...
	self.clearButton = ELib:Button(self,L.MarksClear):Size(100,20):Point("LEFT",self.buttonExport,0,0):Point("TOP",self,0,-30):Tooltip(L.CoinsClear):OnClick(function() 
		StaticPopupDialogs["EXRT_ATTENDANCE_CLEAR"] = {
			...
			button2 = L.NoText,
			OnAccept = function()
				table.wipe(VExRT.Attendance.data)
				UpdateData()
			end,
			timeout = 0,
			whileDead = true,
...
function SaveRaidRoster(encounterID,encounterName,difficultyID,groupSize,isKill)
	...
	VExRT.Attendance.data[#VExRT.Attendance.data + 1] = s
...
local function CheckSpecialConditions(encounterID,encounterName,difficultyID,isKill)
	...
	local conds = {strsplit(";", VExRT.Attendance.enabled)}
	for i=1,#conds do
