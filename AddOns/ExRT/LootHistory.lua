...
function module.main:ADDON_LOADED()
	VExRT = _G.VExRT
	VExRT.LootHistory = VExRT.LootHistory or {}
	VExRT.LootHistory.list = VExRT.LootHistory.list or {}
	VExRT.LootHistory.bossNames = VExRT.LootHistory.bossNames or {}
	VExRT.LootHistory.instanceNames = VExRT.LootHistory.instanceNames or {}
	
	if not VExRT.LootHistory.disable then
		module:Enable()
	end
end
...
function module.main:BOSS_KILL(encounterID, name)
	...
	VExRT.LootHistory.bossNames[encounterID] = name
end
...
function module.main:ENCOUNTER_LOOT_RECEIVED(encounterID, itemID, itemLink, quantity, playerName, className)
	...
	VExRT.LootHistory.instanceNames[instanceID or 0] = instanceName
...
function module.options:Load()
	...
	function self:UpdatePage()
		...
		for i=#VExRT.LootHistory.list,1,-1 do
			local timeRec,encounterID,instanceID,difficulty,playerName,classID,quantity,itemLink = strsplit("#",VExRT.LootHistory.list[i])

			local instanceName = VExRT.LootHistory.instanceNames[tonumber(instanceID)]

			local dateRec = date("%d.%m.%Y %H:%M:%S",tonumber(timeRec))

			encounterID = tonumber(encounterID)
			local encounterName =  VExRT.LootHistory.bossNames[encounterID] or L.bossName[encounterID]
	...
	self.clearButton = ELib:Button(self,L.MarksClear):Size(100,20):Point("BOTTOM",self.decorationLine,"TOP",0,5):Point("RIGHT",self,-10,0):Tooltip(L.EncounterClear):OnClick(function() 
		StaticPopupDialogs["EXRT_LOOTHISTORY_CLEAR"] = {
			...
			button2 = L.NoText,
			OnAccept = function()
				table.wipe(VExRT.LootHistory.list)
				table.wipe(VExRT.LootHistory.bossNames)
				table.wipe(VExRT.LootHistory.instanceNames)
				module.options:UpdatePage()
			end,
			timeout = 0,
			whileDead = true,
	...
	self.chkEnable = ELib:Check(self,L.Enable,not VExRT.LootHistory.disable):Point("LEFT",self.clearButton,"LEFT",-100,0):Size(18,18):AddColorState():OnClick(function(self) 
		if self:GetChecked() then
			VExRT.LootHistory.disable = false
			module:Enable()
		else
			VExRT.LootHistory.disable = true
			module:Disable()
		end
	end)
