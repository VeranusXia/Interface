...
function Board:onUnitTakeDamage(spellID, casterBoardIndex, eventTargetInfo)
    if eventTargetInfo.newHealth == 0 then
        ...
        CMH:log(string.format('|cFFFF7700 %s 杀死 %s |r', self.units[casterBoardIndex].name, self.units[eventTargetInfo.boardIndex].name))
        self.isMissionOver = self:checkMissionOver()
    end
end
...
function Board:getMyTeam()
    ...
    local loseOrGain = lostHP >= 0 and '损失' or '承受'
    local warningText = self.hasRandomSpells and "|cFFFF0000部队拥有随机技能。实际HP数值可能与预测结果不同。|r\n" or ''
    ...
    text = string.format("%s我的部队：\n%s \n\n总共 %s HP = %s", warningText, text, loseOrGain, math.abs(lostHP))
...
function Board:constructResultString()
    ...
    elseif not self.isMissionOver then
        return string.format('|cFFFF0000回合数超过了 %s 。 鹿死谁手，尚未可知|r', self.max_rounds)
    end
