...
local function createPredictButton(frame)
    ...
    local function onEnter(self)
        ...
        GameTooltip_AddNormalLine(GameTooltip, "模拟50次任务战斗，以获得更准确的成功率。（部队有随机技能时可用）")
        GameTooltip:Show()
    end
...
local function createBestDispositionButton(frame)
    ...
    local function onEnter(self)
        ...
        if self:IsEnabled() then
            GameTooltip_AddNormalLine(GameTooltip, "优化调整部队的站位，可最大限度的减少HP损失。")
            GameTooltip_AddColoredLine(GameTooltip, "该操作只考虑指挥台上的部队，不考虑未上场部队。", RED_FONT_COLOR)
        else
            GameTooltip_AddColoredLine(GameTooltip, "如果部队具有随机技能，" .. '则"优化站位"功能不可用。', RED_FONT_COLOR)
        end
    ...
    frame.BestDispositionButton = BestDispositionButton
        ...
        BestDispositionButton:SetText('优化站位')
