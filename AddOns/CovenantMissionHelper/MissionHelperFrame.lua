...
function MissionHelperFrame:updateMissionHeader(missionInfo)
    ...
    self.missionHeader.Duration:SetText('任务时长：' .. tostring(missionInfo.duration))
    if missionInfo.offerTimeRemaining and not missionInfo.canBeCompleted then
        self.missionHeader.OfferTime:SetText('刷新时间：' .. tostring(missionInfo.offerTimeRemaining))
    end
end
